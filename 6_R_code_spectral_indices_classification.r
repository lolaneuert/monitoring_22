...
plotRGB(l1992, r = 1, g = 2, b = 3, stretch = "lin") # create a different stack of bands: so NIR displayed in red, 
# red in green and green in blue channel)
...
plotRGB(l1992, r = 1, g = 2, b = 3, stretch = "lin") # switch the stack
...
# unsupervised classification  = generates clusters based on similar spectral characteristics inherent in the image
# Then, you classify each cluster without providing training samples of your own

# threshold for trees in the 1992 image
d1c <- unsuperClass(l1992, nClasses = 2) # use function ~unsuperClass() to do an unsupervised clustering of Raster* data 
...
# now compare change over time from 1992 to 2006: use function ~data.frame() to create a table 
# containing all frequencies of forest and human impact pixels
...
hist_1992 <- ggplot(proportions, aes(x = landcover, y = percent_1992, color = landcover)) 
          + geom_bar(stat = "identity", fill = "darkseagreen")
hist_1992 # plot it 
# start with function ~ggplot(), then add the name of the object that is to be used as a basis table
# aes = aesthetics: used to specify x, y, and the colors, here x is the landcover, so either forest or human, 
# and y is the percentage of the different covers in the 1992 image, for the color we again use the landcover variable
# use + to add additional elements into the graph: here for histograms we need geom_bar (here specify the type of statistics, 
# here we use "identity" so it is identical to the data in the table (not means, SE etc.), and specify the fill color
      
# repeat the histogram plot for the 2006 image
hist_2006 <-  ggplot(proportions, aes(x = landcover, y = percent_2006, color = landcover)) 
          + geom_bar(stat = "identity", fill = "darkseagreen")
hist_2006

# create a frame with both histograms together without using the function ~mfrow (multiframe) 
# simply by using a + after activating the patchwork package
...
# for this we could also use the function ~grid.arrange from the gridExtra package, 
# specifying the two plots as well as the number of rows
...
# this time we use a new geometry: geom_raster, specify which raster, in this case the dvi and specify the aes, 
# but we need to type mapping in front
...
dvi_gg_1992 <- ggplot() + geom_raster(dvi1992, mapping = aes(x = x, y = y, fill = layer)) 
            + scale_fill_viridis(option = "inferno") + ggtitle ("Multispectral DVI 1992")
...
dvi_gg_2006 <- ggplot() + geom_raster(dvi2006, mapping = aes(x = x, y = y, fill = layer)) 
            + scale_fill_viridis(option = "magma") + # look at different bands using ggplot
...
### NDVI'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
# the range of DVI (8 bit)is -255 to 255, whilst NDVI is -1 to 1, at (16bit) the range of DVI is -65535 to 65535, 
# but NDVI is still -1 to 1
