...
ggplot() + geom_raster(snow, mapping = aes(x = x, y = y, fill = Snow.Cover.Extent)) + scale_fill_viridis(option = "mako") 
         + ggtitle("Snow Cover Extent 21.12.2020")# look at different bands using ggplot
  
# to have a closer look we cut the image down to a specific extention: in this case only europe should be covered
# for this we have to specify the minimum and maximum extent to cover the specified area as elements of an array: 
# min x~-20, max x ~70, min y ~20, max y ~75 -> this are the coordinates at the corners of our subset 
ext <- c(-20, 70, 20, 75)
# now use function ~crop() to cut down the image into a geographical subset
snow_eu <- crop(snow, ext)

# now plot this
pic_snow_eu <- ggplot() + geom_raster(snow_eu, mapping = aes(x = x, y = y, fill = Snow.Cover.Extent)) 
            + scale_fill_viridis(option = "mako") + ggtitle("Snow Cover Extent Europe 21.12.2020") 
