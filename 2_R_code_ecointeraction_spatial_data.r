...
data(meuse) # this is a free dataset inside of the packages sp, 
# it is the measurements of 4 heavy metals in top soil based in Northern Europe
...
meuse # opens the complete dataset in the console(messy): it is a dataframe with 164 rows + additional columns 
# (soil, landuse type, distance etc.)
View(meuse) # opens the dataframe in a new window, just as if you opened it in the environment, function is from package Rcmdr, 
# therefore install.packages("Rcmdr"), library(Rcmdr) first
...
plot(meuse$cadmium, meuse$zinc)  # in order for r to recognize the column names, you can either specify them using $ 
# or teach R the column names beforehand
...
pairs(meuse) # shows mirrored image of all 14 variables: matrix of 14 x 13: 180 potential relationships plotted at once, very functional
