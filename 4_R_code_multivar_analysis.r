...
# now for the multivariate analysis use function ~decorana() (=detrended correspondence analysis, is just like PCA) 
# and assign it to a name using <-
multivar <- decorana(biomes) # this simplifies the data by creating new data dimensions in a matrix, just like in a PCA
...
# then use function ~ordiellipse() to add a circle around those points who are in the same biome, define in which table; 
# type to define which kind of ellipse, here factor = biomes; what colors; kind = ehull to make a specific shape of ellipse; 
# and lwd = linewidth  
attach(biomes_types)
